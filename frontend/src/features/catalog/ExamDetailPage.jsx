          ...
          <div className="text-xs text-slate-400">
            {user?.role === 'TEACHER' || user?.role === 'ADMIN' ? (
              <span className="text-amber-400 font-semibold">
                ⚠️ Tài khoản {user.role === 'ADMIN' ? 'Admin' : 'Giáo viên'} chỉ có quyền quản lý hệ thống và đề thi, không được phép tham gia lượt làm bài thi của học sinh.
              </span>
            ) : user ? (
              'Sẵn sàng? Hệ thống sẽ bắt đầu đếm ngược ngay khi bạn ấn "Bắt Đầu Làm Bài".'
            ) : (
              'Vui lòng đăng nhập tài khoản Học sinh để làm bài.'
            )}
          </div>
          {user?.role === 'TEACHER' || user?.role === 'ADMIN' ? (
            <Button
              ...
              className="bg-slate-800 border-slate-700 text-slate-200 font-bold px-6 shrink-0"
            >
              Chuyển Đến Quản Lý Đề Thi
            </Button>
          ) : user ? (
            <Button
              variant="primary"
              ...
              className="bg-gradient-to-r from-cyan-500 to-sky-600 hover:from-cyan-400 hover:to-sky-500 font-extrabold px-8 shrink-0 shadow-lg shadow-cyan-500/25 border-0"
            >
            ...
            <Button
              ...
              className="bg-gradient-to-r from-cyan-500 to-sky-600 hover:from-cyan-400 hover:to-sky-500 font-extrabold px-8 shrink-0 border-0"
            >
