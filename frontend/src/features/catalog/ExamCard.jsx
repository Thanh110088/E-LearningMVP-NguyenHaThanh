...
export const ExamCard = ({ exam }) => {
  return (
    <div className="group relative rounded-2xl overflow-hidden glass-card flex flex-col h-full border border-slate-800/80 transition-all duration-300">
      {/* Background Image Container (Thumbnail chìm bên dưới, làm nền cho Badges ở trên) */}
      <div className="relative h-48 w-full overflow-hidden bg-slate-900">
        ...
        />
        {/* Dark Gradient Overlay để nổi bật thông tin Badges bên trên */}
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/60 to-transparent" />

        {/* Badges Container (Nối bật ở lớp phía trên - Layer z-10) */}
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 font-medium">
              <Clock className="w-3.5 h-3.5 text-indigo-400" /> {exam.durationMinutes} phút
            </span>
            <span className="flex items-center gap-1 font-medium">
        ...
        <Link
          to={`/exams/${exam.id}`}
          className="w-full mt-2 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-indigo-600/20 text-indigo-300 hover:text-indigo-200 border border-slate-800 hover:border-indigo-500/50 text-xs font-bold transition-all group/btn"
        >
